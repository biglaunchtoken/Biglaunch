...
contract BigLAUNCH is Context, IERC20 {
    ...
    string private _name = "BigLAUNCH";
    string private _symbol = "BLAUNCH";
    ...
    uint256 public _developmentRatio = 300;
    uint256 public _buybackRatio = 500;
    ...
    address payable private _marketingWallet = payable(0x3C0438763709990A58CA20AF78C0C67221796f5c);
    address payable private _developmentWallet = payable(0x660Fc5011E7dABCdb6d04Fe4EDe3856658df4c05);
...
//===============================================================================================================
    ...
    function setRatios(uint256 liquidity, uint256 marketing, uint256 development, uint256 buyback) external onlyOwner {
        require (liquidity + marketing + development + buyback == 100, "Must add up to 100%");
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if(_hasLimits(from, to)) {
            ...
            if(lpPairs[from] && block.timestamp < _liqAddStamp + 30 seconds) {
                address[] memory path = new address[](2);
                path[0] = address(this);
                path[1] = dexRouter.WETH();
                uint256 bnbAmt = dexRouter.getAmountsOut(amount, path)[1];
                require(bnbAmt <= 3 * 10**18, "Can only buy up to 3 BNB at the start!");
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalFee = _liquidityRatio + _marketingRatio + _developmentRatio + _buybackRatio;
        if (totalFee == 0)
            return;
        ...
        uint256 toDevelopment = (((currentBalance - liquidityBalance) * _developmentRatio) / (totalFee - _liquidityRatio));
        ...
        if (toDevelopment > 0) {
            _developmentWallet.transfer(toDevelopment);
        }
    }
    ...
    function enableTrading() public onlyOwner {
        ...
        if (snipeBlockAmt == 0 || snipeBlockAmt > 5) {
            _liqAddBlock = block.number + 500;
            _liqAddStamp = block.timestamp + 1000000;
        } else {
            _liqAddBlock = block.number;
            _liqAddStamp = block.timestamp;
        }
